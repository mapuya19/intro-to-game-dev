...
right = keyboard_check(vk_right) || keyboard_check(ord("A"))
left = keyboard_check(vk_left) || keyboard_check(ord("D"))
...
if(point_distance(x, y, obj_circle_orange.x, obj_circle_orange.y) < 30) {
	if (orange_textbox == noone && keyboard_check_pressed(vk_space)) {
		...
		if (has_key != true) {
			obj_textbox.text = "The key is some way to the east of here."
		}

		if (has_key == true) {
			obj_textbox.text = "Great, my friend will tell you how to escape this place."
		}
	}
}
...
if(point_distance(x, y, obj_circle_cyan.x, obj_circle_cyan.y) < 30) {
	if (blue_textbox == noone && keyboard_check_pressed(vk_space)) {
		...
		if (has_key == true) {
			obj_textbox.text = "The exit is north of here, hidden behind a tree."
		}
	}
}
